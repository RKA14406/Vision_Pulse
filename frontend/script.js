...
function trendValueForAsset(asset = {}) {
  const live = asset.liveQuote || {};
  const raw = live.changePct ?? asset.mockChangePct ?? 0;
  const n = Number(raw);
  return Number.isFinite(n) ? n : 0;
}

function trendPalette(changePct = 0) {
  ...
  if (Number.isFinite(n) && n < 0) {
    return {
      ...
      lineStart: "#ff5c5c",
      lineEnd: "#ff8a8a",
      areaTop: "rgba(255,92,92,0.20)",
      areaBottom: "rgba(255,92,92,0.00)",
      marker: "#ff5c5c"
    };
  }
  ...
  if (Number.isFinite(n) && n > 0) {
    return {
      ...
      lineStart: "#3ddc97",
      lineEnd: "#62f5b8",
      areaTop: "rgba(61,220,151,0.20)",
      areaBottom: "rgba(61,220,151,0.00)",
      marker: "#3ddc97"
    };
  }
  ...
  return {
    ...
    lineStart: "#ffd166",
    lineEnd: "#ffe29a",
    areaTop: "rgba(255,209,102,0.16)",
    areaBottom: "rgba(255,209,102,0.00)",
    marker: "#ffd166"
  };
}
...
function makeSparkline(points = [], changePct = 0) {
  if (!points.length) return "";
  const trend = trendPalette(changePct).name;
  const width = 280;
  const height = 46;
  ...
  return `
    <svg class="sparkline sparkline-${trend}" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-hidden="true">
  ...
  els.assetsGrid.innerHTML = state.assets.map((asset) => {
    ...
    return `
      <article class="card asset-card" data-symbol="${escapeHtml(asset.symbol)}" tabindex="0" role="button" aria-label="Open ${escapeHtml(asset.name)} chart">
        ...
        ${makeSparkline(live.sparkline || asset.sparkline, change)}
        <div class="quote-footnote">
...
function drawAssetChart(asset) {
  ...
  const trend = trendPalette(trendValueForAsset(asset));
  ...
  lineGradient.addColorStop(0, trend.lineStart);
  lineGradient.addColorStop(1, trend.lineEnd);
  ...
  areaGradient.addColorStop(0, trend.areaTop);
  areaGradient.addColorStop(1, trend.areaBottom);
...
function renderPredictions(prediction = null) {
  ...
  const riskLevel = prediction.riskLevel || "Watch";
  const volatility = prediction.volatility || "medium";
  const mainPrediction = prediction.mainPrediction || "";
  const historicalComparison = prediction.historicalComparison || "No historical match found.";
  const reasoning = Array.isArray(prediction.reasoning) ? prediction.reasoning.filter(Boolean) : [];
  const riskWarning = prediction.riskWarning || "Educational only. Not financial advice. Not a buy/sell signal.";

  const reasoningHtml = reasoning.length
    ? `<ul>${reasoning.map(r => `<li>${escapeHtml(r)}</li>`).join("")}</ul>`
    : "";
  ...
  els.predictionsGrid.innerHTML = `
    <article class="card prediction-card selected-result-card compact-prediction-card ${dirClass}">
      ...
      ${mainPrediction ? `<div class="prediction-main-line"><span>Main Prediction</span><strong>${escapeHtml(mainPrediction)}</strong></div>` : ""}
      ...
      <div class="prediction-scoreboard">
        ...
        <div><span>Risk Level</span><strong>${escapeHtml(riskLevel)}</strong></div>
        <div><span>Volatility</span><strong>${escapeHtml(volatility)}</strong></div>
      </div>
      ...
      <div class="prediction-quick-grid">
        <div>
          ...
          ${reasoningHtml ? `<h4 class="prediction-section-label">Reasoning</h4>${reasoningHtml}` : ""}
        </div>
        <div>
          <h4>Historical comparison</h4>
          <p>${escapeHtml(historicalComparison)}</p>
        </div>
      </div>

      <div class="prediction-disclaimer">${escapeHtml(riskWarning)}</div>
    </article>
  `;
}
